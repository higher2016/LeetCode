 ...
 * 
 * Example 1: Input: candies = [1,1,2,2,3,3] Output: 3 <br>
 ...
 * kinds of candies. <br>
 * 
 * Example 2: Input: candies = [1,1,2,3] Output: 2 <br>
...
public class DistributeCandies_575 {
	// The key of this question is number of candies's kind and number of
	// sister's candies
	...
	public static int distributeCandies(int[] candies) {
		Set<Integer> allKind = new HashSet<Integer>();
		for (int candy : candies) {
			if (!allKind.contains(candy)) {
				allKind.add(candy);
			}
		}
		return Integer.min(allKind.size(), candies.length / 2);
	}
}
